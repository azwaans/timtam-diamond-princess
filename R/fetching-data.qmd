...
- The current `diamond.fasta` file used in the analysis has 70
  seqs_calendar, which are all seqs_calendar published in ref: https://doi.org/10.1073/pnas.2006824117, also 
  used Andréoletti, Zwaans et al (2022), as listed in appendix G.
- This describes how this data is processed for analysis and how times are defined in the BEAST2 xml for timtam2.
...
series of unsequenced cases, and excluding sequence DP0321. Times are uniformly distributed within each day: there are 21 sequences on 15.02 (26th day), 23 on 16.02 (27th day) and 29 on 17.02 (28th day). We save this file as diamond.fasta as input for BEAST2. 
...
All times in the xml are specified relative to the time of the last sequence:

```{r}
last_seq_time <- get_present(seqs_calendar, seqs_timed)
```

In absolute time from the first day of the cruise, this is at time 28.965517. Internally, BEAST2 sets this time as 0, and all other times are specified backwards in time in the xml.
It follows that midnight on the last day of sequencing is:
28.965517 - 29.0 = -0.034483

## Assigning times to time series events (disasterTimes)
We assume the population is tested at midday on each day:
Midday on the last day with a sequence:  
-0.034483 + 0.5 = 0.465517

Midday on the following day is:  
-0.034483 - 0.5 = -0.534483

## Assigning times for prevalence estimates
We estimate prevalence at 9AM on each day weekly. 
With the same logic 9AM on day X in relative time corresponds to: 
-0.034483 + X.625 = X.590517

After the last sequence
-0.034483 - X.375 = -X.409483

## Parameter changes can happen at midnight 
As shown above, midnight on any day X before the last sequence corresponds to the following:
X.965517